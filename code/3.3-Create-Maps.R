...
titles <- c("number of individuals per case", 
            ...
            "proportion of dependent persons (children < 14 & elderly 65+) / total", "proportion of dependent children < 14 / total", "proportion of dependent elderly 65 + / total")
...
## inner loop goes through all variables in each country
  ...
  } else
  {
    p.confidence <- ggplot(data.map, aes(x = long, y = lat, group = group)) +
      ...
      geom_polygon(colour = "#707272", aes(fill = error.breaks)) +  
      scale_fill_manual(values = color, guide = guide_legend(title = "Margin of Error"), na.value = "#D7DBDD") +
      coord_equal() +
      theme.base() + # map text and styling
      theme.confidence() + 
      labs(x = NULL, y = NULL, 
